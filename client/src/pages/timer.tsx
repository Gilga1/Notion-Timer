...
export default function TimerPage() {
  ...
  const [selectedProject, setSelectedProject] = useState<NotionProject | null>(
    null,
  );
  ...
  const activeMins =
    activeSessionId && startedAtMs ? (Date.now() - startedAtMs) / 60000 : 0;
    ...
    if (saved) {
      ...
      setSelectedProject(
        saved.selectedProject
          ? ({ ...saved.selectedProject } as NotionProject)
          : null,
      );
      setSelectedTask(
        saved.selectedTask ? ({ ...saved.selectedTask } as NotionTask) : null,
      );
  ...
  const stopMutation = useMutation({
    mutationFn: (id: number) =>
      apiRequest("POST", `/api/sessions/${id}/stop`, {}),
    onSuccess: async (res) => {
      const session: Session = await res.json();
      qc.invalidateQueries({ queryKey: ["/api/sessions/today"] });
      ...
      toast({
        title: "Session saved",
        description: synced
          ? "Time synced to Notion ✓"
          : "Saved locally (Notion not connected)",
      });
    },
    onError: () =>
  ...
  const handleStart = useCallback(() => {
    if (!selectedTask || !selectedProject) {
      toast({
        title: "Select a project and task first",
        variant: "destructive",
      });
      return;
    }
  ...
  useEffect(() => {
    ...
    if (elapsed >= STOPWATCH_NOTIFY_MINS * 60 && !notified45Ref.current) {
      ...
      notifyUser(
        "Time to relax",
        `You've been focused for ${STOPWATCH_NOTIFY_MINS} minutes.`,
      );
      playBeep();
      toast({
        title: "Time for a break",
        description: `You've studied for ${STOPWATCH_NOTIFY_MINS} mins.`,
      });
    }
  ...
  const handlePomStop = useCallback(() => {
    ...
    if (pomodoroEndAtMs) {
      const remaining = Math.max(
        0,
        Math.ceil((pomodoroEndAtMs - Date.now()) / 1000),
      );
    ...
    const label =
      next.type === "work"
        ...
        : next.mins === 5
          ...
          : "Long break - 10 mins";
    toast({
      title: next.type === "work" ? "Focus time" : "Break time",
      description: label,
    });
    ...
    <div className="min-h-screen p-6 md:p-10 max-w-2xl mx-auto">
      ...
      <div className="mb-8">
        ...
        <p className="text-sm text-muted-foreground mt-0.5">
          Track your deep work, synced to Notion
        </p>
      </div>
      ...
      <div className="mb-8 relative">
        ...
        >
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            {selectedTask?.timeSpentMins != null &&
              selectedTask.timeSpentMins > 0 && (
        ...
        >
          ...
          <div className="flex items-center gap-3">
            {!pomodoroRunning ? (
              ...
              >
                <Play className="w-4 h-4" />
                {pomodoroRemaining === currentCycle.mins * 60
                  ? "Start"
                  : "Resume"}
              </button>
            ) : (
          ...
          <div className="w-full pt-2 border-t border-border flex justify-between text-xs text-muted-foreground">
            ...
            <span>
              Total today: {formatTime(Math.round(totalTodayMins * 60))}
            </span>
          </div>
        </div>
      )}
